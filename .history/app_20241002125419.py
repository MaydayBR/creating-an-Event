...
EVENTBRITE_TOKEN = 'YOUR_PERSONAL_OAUTH_TOKEN'
...
# Route to retrieve organization ID
@app.route('/get_organization_id')
def get_organization_id():
    ...
    if response.status_code == 200:
        data = response.json()
        organization_id = data['organizations'][0]['id']  # Extract the first organization ID
        flash(f'Organization ID: {organization_id}', 'success')
    else:
        flash(f'Error retrieving organization ID: {response.json().get("error_description", "Unknown error")}', 'error')
...
def create_event():
    ...
    # Make the request to Eventbrite API (replace 'YOUR_ORGANIZATION_ID' with actual ID)
    response = requests.post(
        f'https://www.eventbriteapi.com/v3/organizations/YOUR_ORGANIZATION_ID/events/',
        headers=headers,
