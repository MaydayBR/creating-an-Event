...
def create_event():
    try:
        ...
        print(f"Event Name: {event_name}, Event Description: {event_description}, Event Time: {event_time}")

        # Convert event time to UTC for Eventbrite API (example: 11:00 AM -> 15:00 UTC)
        event_utc_time = f"2024-10-10T{event_time}:00Z"
        print(f"Converted UTC Time: {event_utc_time}")
        ...
        print(f"Event Data to Send: {json.dumps(event_data, indent=4)}")
        ...
        print(f"Eventbrite Response Status Code: {response.status_code}")
        print(f"Eventbrite Response Content: {response.text}")
